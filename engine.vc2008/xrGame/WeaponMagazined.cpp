...
void CWeaponMagazined::Load	(LPCSTR section)
{
	...
	empty_click_layer = READ_IF_EXISTS(pSettings, r_string, *hud_sect, "empty_click_anm", NULL);
...
{
	if (smart_cast<CWeaponAmmo*>(m_pInventory->Get( m_ammoTypes[m_ammoType].c_str(),false )))
		return	(true);
	else
		for(u32 i = 0; i < m_ammoTypes.size(); ++i)
			if (smart_cast<CWeaponAmmo*>(m_pInventory->Get( m_ammoTypes[i].c_str(),false )))
				return	(true);
	return		(false);
}
...
void CWeaponMagazined::UnloadMagazine(bool spawn_ammo)
{
	...
	for(l_it = l_ammo.begin(); l_ammo.end() != l_it; ++l_it) 
	{
		if(m_pInventory)
		{
			CWeaponAmmo *l_pA = smart_cast<CWeaponAmmo*>(m_pInventory->Get(l_it->first,true));
			if(l_pA) 
			{
...
bool CWeaponMagazined::GetBriefInfo( II_BriefInfo& info )
{
	...
	{
		u32 count = GetAmmoCount(0);
		if ( at_size == 2 )
			count += GetAmmoCount(1);
		if ( at_size == 3 )
			count += GetAmmoCount(2);
