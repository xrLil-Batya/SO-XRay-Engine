...
:fs_pack_writer(NULL),bFast(false),files_list(NULL),folders_list(NULL),bStoreFiles(false),pPackHeader(NULL),config_ltx(NULL)
{
	...
	XRP_MAX_SIZE	= 1024*1024*1920; // bytes (640Mb)
...
void xrCompressor::OpenPack(LPCSTR tgt_folder, int num)
{
	...
	//DUMMY_STUFF* _dummy_stuff_subst		= NULL; //Diesel
	//_dummy_stuff_subst					= g_dummy_stuff; //Diesel
	//g_dummy_stuff						= NULL; //Diesel
	...
	//g_dummy_stuff	= _dummy_stuff_subst; //////////////Diesel
