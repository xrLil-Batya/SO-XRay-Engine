...
float CScriptGameObject::GetWeight	() const
{
	CInventoryItem *inventory_item = smart_cast<CInventoryItem*>(&object());
	if (!inventory_item) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetWeight!");
		return(false);
	}
	return(inventory_item->GetWeight());
}

void CScriptGameObject::SetWeight	(float val)
{
	CInventoryItem *inventory_item = smart_cast<CInventoryItem*>(&object());
	if (!inventory_item) {
	...
	val -= inventory_item->GetWeight();
	inventory_item->ChangeWeight(val);
}
//-------------- Artefacts Restore Type Getters
u8 CScriptGameObject::GetArtefactHitOnTake	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetArtefactHitOnTake!");
		return(false);
	}
	return(art->m_bHitOnTake);
}
u8 CScriptGameObject::GetArtefactRank	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetArtefactRank!");
		return(false);
	}
	return(art->GetAfRank());
}
float CScriptGameObject::GetHealthSpeed	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetHealthSpeed!");
		return(false);
	}
	return(art->m_fHealthRestoreSpeed);
}
float CScriptGameObject::GetRadiationSpeed	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetRadiationSpeed!");
		return(false);
	}
	return(art->m_fRadiationRestoreSpeed);
}
float CScriptGameObject::GetSatietySpeed	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetSatietySpeed!");
		return(false);
	}
	return(art->m_fSatietyRestoreSpeed);
}
float CScriptGameObject::GetPowerSpeed	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetPowerSpeed!");
		return(false);
	}
	return(art->m_fPowerRestoreSpeed);
}
float CScriptGameObject::GetBleedingSpeed	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetBleedingSpeed!");
		return(false);
	}
	return(art->m_fBleedingRestoreSpeed);
}
float CScriptGameObject::GetPsyhealthSpeed	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetPsyhealthSpeed!");
		return(false);
	}
	return(art->m_fPsyHealthRestoreSpeed);
}
float CScriptGameObject::GetAdditionalWeight	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetAdditionalWeight!");
		return(false);
	}
	return(art->m_additional_weight);
}
float CScriptGameObject::GetArtefactWalkAccel	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetArtefactWalkAccel!");
		return(false);
	}
	return(art->m_fWalkAccel);
}
float CScriptGameObject::GetArtefactJumpSpeed	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetArtefactJumpSpeed!");
		return(false);
	}
	return(art->m_fJumpSpeed);
}
//-------------- Artefacts Immunities Getters
float CScriptGameObject::GetBurnImmunity	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetBurnImmunity!");
		return(false);
	}
	return(art->m_ArtefactHitImmunities[ALife::eHitTypeBurn]);
}
float CScriptGameObject::GetStrikeImmunity	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetStrikeImmunity!");
		return(false);
	}
	return(art->m_ArtefactHitImmunities[ALife::eHitTypeStrike]);
}
float CScriptGameObject::GetShockImmunity	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetShockImmunity!");
		return(false);
	}
	return(art->m_ArtefactHitImmunities[ALife::eHitTypeShock]);
}
float CScriptGameObject::GetWoundImmunity	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetWoundImmunity!");
		return(false);
	}
	return(art->m_ArtefactHitImmunities[ALife::eHitTypeWound]);
}
float CScriptGameObject::GetRadiationImmunity	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetRadiationImmunity!");
		return(false);
	}
	return(art->m_ArtefactHitImmunities[ALife::eHitTypeRadiation]);
}
float CScriptGameObject::GetTelepaticImmunity	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetTelepaticImmunity!");
		return(false);
	}
	return(art->m_ArtefactHitImmunities[ALife::eHitTypeTelepatic]);
}
float CScriptGameObject::GetChemicalBurnImmunity	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetChemicalBurnImmunity!");
		return(false);
	}
	return(art->m_ArtefactHitImmunities[ALife::eHitTypeChemicalBurn]);
}
float CScriptGameObject::GetExplosionImmunity	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetExplosionImmunity!");
		return(false);
	}
	return(art->m_ArtefactHitImmunities[ALife::eHitTypeExplosion]);
}
float CScriptGameObject::GetFireWoundImmunity	() const
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member GetFireWoundImmunity!");
		return(false);
	}
	return(art->m_ArtefactHitImmunities[ALife::eHitTypeFireWound]);
}
//-------------- Artefacts Restore Type Setters
void CScriptGameObject::SetArtefactHitOnTake	(u8 val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetArtefactHitOnTake!");
		return;
	}
	art->m_bHitOnTake = val;
}
void CScriptGameObject::SetArtefactRank	(u8 val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetArtefactRank!");
		return;
	}
	art->SetAfRank(val);
}
void CScriptGameObject::SetHealthSpeed	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetHealthSpeed!");
		return;
	}
	art->m_fHealthRestoreSpeed = val;
}
void CScriptGameObject::SetRadiationSpeed	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetRadiationSpeed!");
		return;
	}
	art->m_fRadiationRestoreSpeed = val;
}
void CScriptGameObject::SetSatietySpeed	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetSatietySpeed!");
		return;
	}
	art->m_fSatietyRestoreSpeed = val;
}
void CScriptGameObject::SetPowerSpeed	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetPowerSpeed!");
		return;
	}
	art->m_fPowerRestoreSpeed = val;
}
void CScriptGameObject::SetBleedingSpeed	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetBleedingSpeed!");
		return;
	}
	art->m_fBleedingRestoreSpeed = val;
}
void CScriptGameObject::SetPsyhealthSpeed	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetPsyhealthSpeed!");
		return;
	}
	art->m_fPsyHealthRestoreSpeed = val;
}
void CScriptGameObject::SetAdditionalWeight	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetAdditionalWeight!");
		return;
	}
	art->m_additional_weight = val;
}
void CScriptGameObject::SetArtefactWalkAccel	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetArtefactWalkAccel!");
		return;
	}
	art->m_fWalkAccel = val;
}
void CScriptGameObject::SetArtefactJumpSpeed	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetArtefactJumpSpeed!");
		return;
	}
	art->m_fJumpSpeed = val;
}
//-------------- Artefacts Immunities Setters
void CScriptGameObject::SetBurnImmunity	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetBurnImmunity!");
		return;
	}
	art->m_ArtefactHitImmunities[ALife::eHitTypeBurn] = val;
}
void CScriptGameObject::SetStrikeImmunity	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetStrikeImmunity!");
		return;
	}
	art->m_ArtefactHitImmunities[ALife::eHitTypeStrike] = val;
}
void CScriptGameObject::SetShockImmunity	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetShockImmunity!");
		return;
	}
	art->m_ArtefactHitImmunities[ALife::eHitTypeShock] = val;
}
void CScriptGameObject::SetWoundImmunity	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetWoundImmunity!");
		return;
	}
	art->m_ArtefactHitImmunities[ALife::eHitTypeWound] = val;
}
void CScriptGameObject::SetRadiationImmunity	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetRadiationImmunity!");
		return;
	}
	art->m_ArtefactHitImmunities[ALife::eHitTypeRadiation] = val;
}
void CScriptGameObject::SetTelepaticImmunity	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetTelepaticImmunity!");
		return;
	}
	art->m_ArtefactHitImmunities[ALife::eHitTypeTelepatic] = val;
}
void CScriptGameObject::SetChemicalBurnImmunity	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetChemicalBurnImmunity!");
		return;
	}
	art->m_ArtefactHitImmunities[ALife::eHitTypeChemicalBurn] = val;
}
void CScriptGameObject::SetExplosionImmunity	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetExplosionImmunity!");
		return;
	}
	art->m_ArtefactHitImmunities[ALife::eHitTypeExplosion] = val;
}
void CScriptGameObject::SetFireWoundImmunity	(float val)
{
	CArtefact *art = smart_cast<CArtefact*>(&object());
	if (!art) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member SetFireWoundImmunity!");
		return;
	}
	art->m_ArtefactHitImmunities[ALife::eHitTypeFireWound] = val;
}
//-------------- Artefacts End
