...
bool CUIActorMenu::OnItemDrop(CUICellItem* itm)
{
	...
	if (old_owner == new_owner)
    {
		...
		if (ai().script_engine().functor("actor_callbacks.CUIActorMenu_OnItemDropped", funct)) 
		{
			...
			PIItem _iitem1 = itm ? (PIItem)itm->m_pData : NULL;

			CGameObject* GO1 = _iitem1 ? smart_cast<CGameObject*>(_iitem1) : NULL;
			CGameObject* GO2 = _iitem ? smart_cast<CGameObject*>(_iitem) : NULL;
			if (GO1 && GO2){
			funct(GO1->lua_game_object(), GO2->lua_game_object(), (int)t_old, (int)t_new);
			}
        }
        return false;
    }
...
bool CUIActorMenu::OnItemDbClick(CUICellItem* itm)
{
    ...
    case iActorBag:
    {
        ...
        else if (!(iitem_to_place->BaseSlot()>=BOUCH_UP_1&&iitem_to_place->BaseSlot()<=ZAYAZ_GREN) && !ToSlot(itm, false, iitem_to_place->BaseSlot()))
            if (!ToBelt(itm, false))
...
bool CUIActorMenu::OnItemFocusReceive(CUICellItem* itm)
{
    ...
    luabind::functor<void> funct;
    if (ai().script_engine().functor("actor_callbacks.CUIActorMenu_OnItemFocusReceive", funct))
    {
