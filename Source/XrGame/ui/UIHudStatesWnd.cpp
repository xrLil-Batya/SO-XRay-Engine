...
void CUIHudStatesWnd::InitFromXml( CUIXml& xml, LPCSTR path )
{
	...
	m_ui_grenades				= UIHelper::CreateTextWnd( xml, "static_grens", this );
	m_ui_medcine				= UIHelper::CreateTextWnd( xml, "static_medcin", this );
	m_ui_name					= UIHelper::CreateTextWnd( xml, "static_name_ammo", this );
...
void CUIHudStatesWnd::ShowHud()
{
	CActor* actor = smart_cast<CActor*>( Level().CurrentViewEntity() );
	if ( !actor )
		return;

	m_timer_showing = Device.dwTimeGlobal + 4000;
	m_back->Show(true);
...
void CUIHudStatesWnd::Update()
{
	...
	if (Device.dwTimeGlobal <= m_timer_showing){
		UpdateActiveItemInfo( actor );
	}
	else{
		UpdateActiveItemInfo( actor );
		...
		m_ui_medcine->Show(false);
		m_ui_grenades->Show(false);
		m_ui_name->Show(false);
	}

	PIItem detector = actor->inventory().ItemFromSlot(ZAYAZ_DOSIMETER);
	if (detector) {
		shared_str sect_name = detector->object().cNameSect();
		LPCSTR zone_config = pSettings->r_string(sect_name, "detector_config");
		Level().hud_zones_list->load( zone_config, "zone" );
	}
	else {
		Level().hud_zones_list->load( "none_zone_detector", "zone" );
	}
...
{
	string32	int_str;
	m_ui_grenades->Show			(true);
	m_ui_medcine->Show			(true);

	u32 count = actor->inventory().dwfGetZayazCount(2);
	xr_sprintf( int_str, "%d", count );
	m_ui_grenades->SetText( int_str );
	count = actor->inventory().dwfGetZayazCount(1);
	xr_sprintf( int_str, "%d", count );
	m_ui_medcine->SetText( int_str );
	...
	if ( item )
	{
		...
		m_ui_name->SetText				( m_item_info.name.c_str() );
	...
	else
	{
		...
		count = actor->inventory().dwfGetZayazCount(3);
		xr_sprintf( int_str, "%d", count );
		m_ui_weapon_fmj_ammo->SetText( int_str );
		count = actor->inventory().dwfGetZayazCount(4);
		xr_sprintf( int_str, "%d", count );
		m_ui_grenade->SetText( int_str );
		m_ui_weapon_fmj_ammo->SetTextColor(color_rgba(238,155,23,255));
		m_ui_grenade->SetTextColor(color_rgba(238,155,23,255));
	}
}
