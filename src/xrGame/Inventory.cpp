...
{
	/** НЕ применять для ванильных слотов. Этот метод не заменяет итем при уже занятом m_pIItem */
	if (item == NULL) return;

	if (m_pIItem == item)
		return; // у нас уже есть такой итем
...
//найти в инвентаре вещь с указанным именем
